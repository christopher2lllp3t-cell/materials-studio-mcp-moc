...
if ([string]::IsNullOrWhiteSpace($ReceiptPath)) {
    $ReceiptPath = Join-Path $sourceRoot "docs\validation\receipts\p2-castep-runner-qualification-verification.json"
}
...
foreach ($required in @($sourcePython, $deploymentPython, $sourceManifest)) {
    if (-not (Test-Path -LiteralPath $required -PathType Leaf)) {
        throw "Required P2 candidate verification file is missing: $required"
    }
}
...
if ($sourceIntegrity.status -ne "pass" -or $sourceIntegrity.release_version -ne "1.3.0") {
    throw "Source release manifest is not a passing 1.3.0 P2 candidate verification"
}
...
$receipt = [ordered]@{
    schema_version = 1
    verification_entry = "scripts/verify_candidate_p2.ps1"
    candidate = [ordered]@{
        qualification = "p2_private_synthetic_standalone_castep_runner"
...
Write-Output ("P2_CANDIDATE_VERIFICATION_PASS receipt=" + (Resolve-Path -LiteralPath $ReceiptPath).Path)
